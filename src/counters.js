...
async function updateGlobalCounterAndLog(client, name, user, guildId, increment) {
  ...
  await sendLogMessage(client, `Global counter **"${name}"** ${increment > 0 ? 'incremented' : 'decremented'} for member ${getUserTag(user)}, they now have ${counter}.`);
  return counter;
}
...
async function updateStageCounterAndLog(client, stageId, name, user, guildId, increment) {
  ...
  await sendLogMessage(client, `${stageId} counter **"${name}"** ${increment > 0 ? 'incremented' : 'decremented'} for member ${getUserTag(user)}, they now have ${counter}.`);
  return counter;
}
